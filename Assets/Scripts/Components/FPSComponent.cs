...
public class FPSComponent : NetworkBehaviour
{
    ...
    private void Start()
    {
        ...
        m_Camera = cameraChild.GetComponent<Camera>();
    ...
    public void CmdFireGun(){
        if(!currentGun.projectile){
            RaycastHit hit;
            if(Physics.Raycast(
                cameraChild.transform.position,
                cameraChild.transform.forward,
                out hit,
                currentGun.maxRange)
            ){
                ...
                } // TODO make a different sprite based on tag of what we hit, i.e. blood

                GameObject hitsprite = Instantiate(GenericGunHitSprite);
                hitsprite.transform.position = Vector3.Lerp(transform.position, hit.point, 0.95f);
                NetworkServer.Spawn(hitsprite);
            }
        } else {
            GameObject projectile = GameObject.Instantiate(currentGun.projectilePrefab);

            projectile.transform.position = cameraChild.transform.position;
            projectile.transform.rotation = cameraChild.transform.rotation;
